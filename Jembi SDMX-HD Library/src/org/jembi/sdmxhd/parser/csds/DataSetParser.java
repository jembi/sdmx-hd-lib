...
public class DataSetParser {
	...
	public DataSet parse(XMLEventReader eventReader, StartElement se,
		...
		while (!endTagReached) {
			...
			if (event.isStartElement()) {
				...
				if (se.getName().getLocalPart().equalsIgnoreCase("Group")
						&& se.getName().getNamespaceURI().equalsIgnoreCase(
								derivedNamespace)) {
					// read Group
					Group g = processGroup(se, eventReader, derivedNamespace);
					d.getGroups().add(g);
				}
	...
	private Group processGroup(StartElement se, XMLEventReader eventReader,
		...
		while (attributes.hasNext()) {
			Attribute attr = attributes.next();
			g.addAttribute(attr.getName().getLocalPart(), attr.getValue());
		}
		...
		while (!endTagReached) {
			...
			if (event.isStartElement()) {
				se = event.asStartElement();
				if (se.getName().getLocalPart().equalsIgnoreCase("Section")
						&& se.getName().getNamespaceURI().equalsIgnoreCase(
								derivedNamespace)) {
					// read Section
					Section s = processSection(se, eventReader,
							derivedNamespace);
					g.getSections().add(s);
				}
			} else if (event.isEndElement()) {
	...
	private Section processSection(StartElement se, XMLEventReader eventReader,
			String derivedNamespace) throws XMLStreamException {
		...
		while (!endTagReached) {
			...
			if (event.isStartElement()) {
				...
				EndElement ee = event.asEndElement();
				if (ee.getName().getLocalPart().equalsIgnoreCase("Section")
						&& ee.getName().getNamespaceURI().equalsIgnoreCase(
								derivedNamespace)) {
					endTagReached = true;
				}
