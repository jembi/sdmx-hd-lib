...
public class HierarchyTest {
	...
	public void testFindCodeRef() throws Exception {
		...
		Hierarchy hierarchy = dsd.getHierarchicalCodelists().get(0)
				.getHierarchys().get(0);
		CodeRef codeRef = hierarchy.findCodeRef("AL_TIME_PERIOD", "180");

		Assert.assertNotNull(codeRef);
		Assert.assertEquals("180", codeRef.getCodeID());
		Assert.assertEquals("AL_TIME_PERIOD", codeRef.getCodelistAliasRef());
	}
